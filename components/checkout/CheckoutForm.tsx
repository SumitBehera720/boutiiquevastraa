    ...
    if (cleanZip.length === 6 && /^\d{6}$/.test(cleanZip)) {
      ...
      const lookupPincode = async () => {
        ...
        try {
          ...
          if (isMounted) {
            setPinError(""); // Silent error: allow manual entry
          }
        } finally {
              ...
              <div className="pt-4 border-t border-gray-100">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-gray-500 uppercase mb-1.5 flex items-center justify-between">
                        <span>Pincode / ZIP</span>
                        {pinLoading && (
                          <span className="text-[10px] text-maroonClr font-normal flex items-center gap-1">
                      ...
                      <input
                        ...
                        className="w-full border border-gray-300 rounded px-4 py-2.5 text-sm focus:outline-none focus:border-maroonClr focus:ring-1 focus:ring-maroonClr font-mono"
                      />
                      {pinError && (
                        <p className="mt-1 text-[11px] text-red-600 font-sans flex items-center gap-1 font-medium">
                          <AlertCircle className="w-3 h-3 text-red-500" />
                          {pinError}
                        </p>
                      )}
                      {zip.trim().length === 6 && /^\d{6}$/.test(zip.trim()) && !pinError && (
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          Instant Payment via Razorpay (UPI, Cards, NetBanking, Wallets & QR)
                        </p>
                      </div>
                    </div>
